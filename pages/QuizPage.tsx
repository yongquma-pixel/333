...
import { CheckCircle, XCircle, RefreshCw, Trophy, Settings2, Target, Edit3, ArrowRight, BrainCircuit, CalendarClock, Flame } from 'lucide-react';
...
export const QuizPage: React.FC = () => {
  ...
  const handleAnswer = async (option: string) => {
    ...
    // Process Result (SRS + Mistake Logic)
    ...
    return (
      <div className="flex flex-col items-center justify-center h-full p-6 text-center space-y-8 animate-fade-in-up">
        ...
        <div>
            ...
            {isReviewMode && (
                <div className="mt-2 inline-block px-4 py-1 bg-amber-100 text-amber-700 rounded-full text-xs font-bold">
                    {dueCount} 个待复习
                </div>
            )}
            {isMistakeMode && (
                <div className="mt-2 inline-block px-4 py-1 bg-red-100 text-red-700 rounded-full text-xs font-bold">
                    {mistakeCount} 个重点攻克
                </div>
            )}
        </div>
        ...
        <button 
            ...
            className={`w-full max-w-xs text-white py-4 rounded-xl font-bold shadow-lg disabled:opacity-50 flex items-center justify-center space-x-2 ${isMistakeMode ? 'bg-red-600' : isReviewMode ? 'bg-amber-600' : 'bg-brand-600'}`}
        >
            <span>开始</span>
  ...
  if (gameState === 'result') {
    ...
    return (
      <div className="p-6 h-full overflow-y-auto pb-20">
        ...
        {wrongAnswers.length > 0 && <div className="space-y-3"><h3 className="font-bold flex items-center"><XCircle className="w-4 h-4 text-red-500 mr-1" />本次错题</h3>{wrongAnswers.map((w, idx) => (<div key={idx} className="bg-red-50 p-4 rounded-xl flex justify-between items-center"><div>{w.streetName}</div><span className="font-bold text-red-600">{w.routeArea}</span></div>))}</div>}
  ...
  return (
    <div className="h-full flex flex-col p-6">
      ...
      <div className="flex-1 flex flex-col justify-center">
        ...
        <div className="mb-2 flex items-center space-x-2">
            {isMistakeMode && (
                <div className="bg-red-100 text-red-600 text-[10px] font-bold px-2 py-0.5 rounded-full flex items-center">
                    ...
                    连续答对: {currentQ.question.mistakeStreak || 0} / 5
                </div>
            )}
            {isReviewMode && <span className="bg-amber-100 text-amber-600 text-[10px] font-bold px-2 py-0.5 rounded-full">遗忘曲线复习</span>}
        </div>
        ...
        {/* Manual Next Button only appears when wrong */}
        {isAnswered && selectedOption !== currentQ.question.routeArea && (
            <button onClick={nextQuestion} className="mt-6 w-full bg-brand-600 text-white py-3 rounded-xl font-bold flex justify-center items-center shadow-lg animate-fade-in-up">
